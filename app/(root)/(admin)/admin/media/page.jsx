      ...
      <Card className='py-0 rounded shadow-sm border' suppressHydrationWarning={true}>
        <CardHeader className='pt-3 px-3 pb-1' style={{ borderBottom: '1px solid #e5e7eb' }} suppressHydrationWarning={true}>
          <div className='flex justify-between items-center' suppressHydrationWarning={true}>
            <h4 className='font-semibold text-xl uppercase' suppressHydrationWarning={true}>{deleteType === 'SD' ? 'Media': 'Media Trash'}</h4>
            <div className='flex items-center gap-5' suppressHydrationWarning={true}>
              {deleteType === 'SD' && <UploadMedia></UploadMedia>}

              <div className='flex gap-3' suppressHydrationWarning={true}>
                {
                  deleteType === 'SD' ? 
                  <Button asChild variant={'destructive'} suppressHydrationWarning={true}>
              ...
              <div className='flex gap-2' suppressHydrationWarning={true}>
                {
                  deleteType === 'SD' 
                  ? 
                  <Button variant={'destructive'} onClick={()=>handleDelete(selectedMedia, deleteType)} suppressHydrationWarning={true}>
                    Move Into Trash
                  </Button>
                  :
                  <>
                  <Button className={'bg-green-500 hover:bg-green-600'} onClick={()=>handleDelete(selectedMedia, "RSD")} suppressHydrationWarning={true}>
                    Restore
                  </Button>
                   <Button variant={'destructive'} onClick={()=>handleDelete(selectedMedia, deleteType)} suppressHydrationWarning={true}>
                   Delete Permanently
